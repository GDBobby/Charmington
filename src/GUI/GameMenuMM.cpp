...
namespace EWE {
	GameMenuMM::GameMenuMM(EWEDevice& device, float screenWidth, float screenHeight) {
		...
		clickText.emplace_back("Graphics Settings", screenWidth / 2.f, 600.f * heightRescaling, TA_center, 3.f, screenWidth, screenHeight);
		clickText.emplace_back("Audio Settings", screenWidth / 2.f, 680.f * heightRescaling, TA_center, 3.f, screenWidth, screenHeight);

		clickText.emplace_back("Exit", screenWidth / 2.f, 860.f * heightRescaling, TA_center, 3.f, screenWidth, screenHeight);
		clickText.emplace_back("Return to Main", screenWidth / 2.f, 520.f * heightRescaling, TA_center, 3.f, screenWidth, screenHeight);
		clickText.emplace_back("Resume", screenWidth / 2.f, 440.f * heightRescaling, TA_center, 3.f, screenWidth, screenHeight);
		//clickText.emplace_back("Level Builder", 0.f, 760.f * heightRescaling, TA_left, 3.f, screenWidth, screenHeight);

	}

	void GameMenuMM::processClick(double xpos, double ypos) {
		...
		if (returnValues.first == UIT_ClickTextBox) {
			...
			case 4: {
				clickReturns.push(MCR_CharmingtonResume);
				break;
			}
